/*
From github.com/getlantern/systray
...
func (item *MenuItem) SetInfo(title string, tooltip string, checked bool, checkbox bool, disable bool) {
	item.title = title
	item.tooltip = tooltip
	item.checked = checked
	item.isCheckable = checkbox
...
func systrayMenuItemSelected(id uint32) {
	...
	if !ok {
		fmt.Printf("[tray]No menu item with ID %v\n", id)
		return
	}
	select {
